<div class="main-section">
    ...
    <div class="row text-center form-group btn-group-st">
        <a class="btn btn-info btn-sm {% if page.current == 1 %}disabled{% endif %}" href="{{ url.get(['for':'purchases'])|e }}">صفحه اول</a>
        <a class="btn btn-info btn-sm {% if page.current == page.before %}disabled{% endif %}" href="{{ url.get(['for':'purchases','page':page.before])|e }}"><i class="fa fa-chevronright"></i> صفحه قبل</a>
        <a class="btn btn-info btn-sm {% if page.current == page.next %}disabled{% endif %}" href="{{ url.get(['for':'purchases','page':page.next])|e }}">صفحه بعد <i class="fa fa-chevron-left"></i></a>
        <a class="btn btn-info btn-sm {% if page.current == page.last %}disabled{% endif %}" href="{{ url.get(['for':'purchases','page':page.last])|e }}">صفحه آخر</a>
    </div>
</div>